...
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr" lang="<?php echo str_replace('_', '-', osc_current_user_locale()); ?>">
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $('form#change-username').validate({
                    ...
                    messages: {
                        s_username: {
                            required: '<?php echo osc_esc_js(__("Username: this field is required", "paris")); ?>.'
                        }
                    },
                        ...
                        cInterval = setInterval(function(){
                            $.getJSON(
                                ...
                                function(data){
                                    clearInterval(cInterval);
                                    if(data.exists==0) {
                                        $("#available").text('<?php echo osc_esc_js(__("The username is available", "paris")); ?>');
                                    } else {
                                        $("#available").text('<?php echo osc_esc_js(__("The username is NOT available", "paris")); ?>');
                                    }
                                }
                            );
                        }, 1000);
  ...
  <div class="panel-heading">
                <h1><?php _e('Change your username', 'paris'); ?></h1></div>
  <div class="panel-body">
                ...
                <form id="change-username" action="<?php echo osc_base_url(true); ?>" method="post">
                    ...
                    <fieldset>
                        <p>
                            <label for="s_username"><?php _e('Username', 'paris'); ?></label>
                            <input type="text" name="s_username" id="s_username" value="" />
                        </p>
                        <p>
                            <span class="help-box" ><?php _e('WARNING: Once set, you will not be able to change your username again. Choose wisely.', 'paris'); ?></span>
                        </p>
